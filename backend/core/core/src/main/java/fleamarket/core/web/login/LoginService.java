...
public class LoginService {
    ...
    public List<ItemDTO> getItems(HttpServletRequest request){
        ...
        items.stream().forEach(item -> itemDTOs.add(new ItemDTO(realMember.getName(),item.getItemId(),item.getItemName(),item.getDescription(), item.getPrice(),item.isReserved(),item.isSoldOut())));
        return itemDTOs;
    }
}
